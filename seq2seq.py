...
def _extract_argmax_and_embed(embedding,
  ...
  """Get a loop_function that extracts the previous symbol and embeds it.
     返回一个函数，该函数的功能是根据之前的输出，根据投影层转换成输出向量，然后找出输出向量
     值最大的词，然后根据embedding 找初该词的embedding词向量
  ...
  def loop_function(prev, _):
    ...
    # embedding_lookup.
    # 根据embedding matrix 找出prev_symbol对应的embedding 词向量
...
def attention_decoder(decoder_inputs,# T * [batch_size,input_size]
                      ...
                      attention_states,# [batch_size,T,cell.output_size]
                      cell,
                      output_size=None,
                      num_heads=1,
                      loop_function=None,
  ...
  with variable_scope.variable_scope(
    ...
    # num_heads:attention是对信息的加权求和，一个attention head对应了一种加权求和的方式
    # num_heads:定义了用多少个attention head加权求和
...
def embedding_attention_decoder(decoder_inputs,
                                ...
                                embedding_size,
                                num_heads=1,
                                output_size=None,# 输出的大小，num_decoder_symbols
                                output_projection=None,
                                feed_previous=False,
                                update_embedding_for_previous=True,
                                dtype=None,
  ...
  """RNN decoder with embedding and attention and a pure-decoding option.
     带有attention机制，embedding机制的decoder
  ...
  with variable_scope.variable_scope(
    ...
    # 获取embedding matrix矩阵 [num_symbols, embedding_size]
    embedding = variable_scope.get_variable("embedding",
                                            [num_symbols, embedding_size])
    # 根据输出向量获取最大词并获得该词的embedding向量的功能
    loop_function = _extract_argmax_and_embed(
        embedding, output_projection,
        update_embedding_for_previous) if feed_previous else None

    # 将decoder inputs 转换成词embedding向量的输入形式
    emb_inp = [
...
def embedding_attention_seq2seq(encoder_inputs,# [T,batch_size]
                                ...
                                cell,
                                num_encoder_symbols,#编码符号数
                                num_decoder_symbols,#解码符号数
                                embedding_size,#词向量的维度
                                num_heads=1,#抽头数量
                                output_projection=None,#decoder的output向量投影到词表空间时，用到的投影矩阵和偏置项（W,b）
                                feed_previous=False,#若为True,只有decoder的第一个输入<go>符号有用，所有的decoder的输入都依赖于上一步输出，在预测时用到
                                dtype=None,
                                scope=None,
                                initial_state_attention=False #默认为False,初始的attention为0，若为True，将从initial state和attention states开始attention
                                ):
  ...
  Returns:
    A tuple of the form (outputs, state), where:
      outputs: A list of the same length as decoder_inputs of 2D Tensors with
        ...
        outputs.
      outputs 是2D Tensors list,每个Tensor的shape是[batch_size,cell.state_size]
      state: The state of each decoder cell at the final time-step.
        It is a 2D Tensor of shape [batch_size x cell.state_size].
      state:是最后一个时间步,shape是[batch_size,cell.state_size]
  """
  with variable_scope.variable_scope(
    ...
    # Encoder.
    encoder_cell = copy.deepcopy(cell)
    # 使用EmbeddingWrapper将输入映射到embedding_size大小的向量
    encoder_cell = core_rnn_cell.EmbeddingWrapper(
        encoder_cell,
        embedding_classes=num_encoder_symbols,
        embedding_size=embedding_size)
    #通过调用static_rnn 得到encoder的每一个时刻的输出和状态
    encoder_outputs, encoder_state = rnn.static_rnn(
        encoder_cell, encoder_inputs, dtype=dtype)
    #enoder_outputs.shape:[T,batch_size,cell.output_size]
    #encoder_state.shape:[batch_size,cell.output_size]

    # First calculate a concatenation of encoder outputs to put attention on.
    top_states = [
        array_ops.reshape(e, [-1, 1, cell.output_size]) for e in encoder_outputs
    ]
    # top_states.shape:T * [batch_size,1,cell.output_size]
    attention_states = array_ops.concat(top_states, 1)
    # attention_states.shape:[batch_size,T,cell.output_size]
    # Decoder.
    output_size = None
    if output_projection is None:
      # 如果未指定output_projection,通过OutputProjectionWrapper将输出映射成词向量的维度
      cell = core_rnn_cell.OutputProjectionWrapper(cell, num_decoder_symbols)
      output_size = num_decoder_symbols

    if isinstance(feed_previous, bool):
      # 调用embedding_attention_decoder
